...
String matteus = "select to_char(pe.ped_data,'dd/mm/yyyy'),c.cli_cidade,c.cli_estado,c.cli_nome,f.for_nome,to_char((select sum(it.item_valor * it.item_quantidade) \n" +
...
"c.cli_cidade ilike 'cruzeiro' AND pe.ped_status ilike 'pago' AND pe.ped_pagamento ilike 'cartão' AND pe.ped_pedido = 'SO'),'R$09G999D99') as total,\n" +
"to_char(((select sum(it.item_valor * it.item_quantidade) from pedidos as pe inner join clientes as c on pe.fk_cliente = c.cli_id\n" +
...
"pe.ped_pedido = 'SO')*c.cli_frete)/100,'R$09G999D99') as frete,pe.ped_pagamento,pe.ped_pedido,pe.ped_status from pedidos pe inner join \n" +
